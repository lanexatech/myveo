...
import { AspectRatio, Resolution, VideoOptions, PromptFormData } from './types';
...
const VideoGenerator: React.FC<VideoGeneratorProps> = ({ prompt, setPrompt }) => {
                    ...
                    <div className="space-y-4">
                       <div className="flex items-center gap-3">
                            ...
                            <div className="flex gap-2 p-1 bg-indigo-100/50 rounded-lg">
                                <OptionButton value="16:9" selectedValue={options.aspectRatio} onClick={val => setOptions(o => ({ ...o, aspectRatio: val as AspectRatio }))}>16:9</OptionButton>
                                <OptionButton value="9:16" selectedValue={options.aspectRatio} onClick={val => setOptions(o => ({ ...o, aspectRatio: val as AspectRatio }))}>9:16</OptionButton>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            <span className="text-sm font-medium text-indigo-800 w-24">Resolution</span>
                             <div className="flex gap-2 p-1 bg-indigo-100/50 rounded-lg">
                                <OptionButton value="720p" selectedValue={options.resolution} onClick={val => setOptions(o => ({ ...o, resolution: val as Resolution }))}>720p</OptionButton>
                                <OptionButton value="1080p" selectedValue={options.resolution} onClick={val => setOptions(o => ({ ...o, resolution: val as Resolution }))}>1080p</OptionButton>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
...
const initialPromptFormData: PromptFormData = {
  idePrompt: '',
  subjek: '',
  usia: '',
  warnaKulit: '',
  ...
  kalimatDiucapkan: '',
  detailTambahan: '',
  negativePrompt: 'blur, low quality, watermark, signature, ugly, deformed, text, label',
};

const App: React.FC = () => {
    const [activeTab, setActiveTab] = useState<'videoGenerator' | 'promptGenerator'>('videoGenerator');
    
    // State for the main video prompt, lifted up from VideoGenerator
    const [videoPrompt, setVideoPrompt] = useState<string>('A photorealistic video of a cat wearing sunglasses, riding a skateboard on Mars.');

    // State for PromptGenerator lifted to App component
    const [promptFormData, setPromptFormData] = useState<PromptFormData>(initialPromptFormData);
    ...
    return (
        <div className="min-h-screen w-full bg-gradient-to-br from-indigo-50 to-purple-100 p-4 sm:p-6 lg:p-8 flex items-center justify-center font-sans">
            <main className="w-full max-w-6xl mx-auto bg-white/50 backdrop-blur-xl border border-white/30 rounded-2xl shadow-2xl shadow-indigo-200/50 overflow-hidden">
                <div className="p-6 border-b border-white/30 flex flex-col sm:flex-row justify-between items-start sm:items-center">
                    <div>
                     ...
                     <div className="flex-shrink-0 mt-4 sm:mt-0 flex gap-2 p-1 bg-indigo-100/50 rounded-lg">
                        <button 
                            onClick={() => setActiveTab('videoGenerator')}
                            className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${activeTab === 'videoGenerator' ? 'bg-indigo-500 text-white shadow' : 'text-indigo-800 hover:bg-indigo-200/50'}`}
                        >
                            Generator Video
                        </button>
                        <button 
                            onClick={() => setActiveTab('promptGenerator')}
                            className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${activeTab === 'promptGenerator' ? 'bg-indigo-500 text-white shadow' : 'text-indigo-800 hover:bg-indigo-200/50'}`}
                        >
                            Generator Prompt
                        </button>
                    </div>
                ...
                {activeTab === 'videoGenerator' && <VideoGenerator prompt={videoPrompt} setPrompt={setVideoPrompt} />}
                {activeTab === 'promptGenerator' && (
                  <PromptGenerator 
                    ...
                    setEnglishPrompt={setEnglishPrompt}
